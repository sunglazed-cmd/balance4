...
let today = { foods: [], workouts: [], steps: 0, walkMs: 0 };
let currentDayKey = null; // date key that `today` currently holds data for
let historyCache = {}; // dateKey -> day record
let currentTab = 'today';
let genderVal = 'm', activityVal = 1.375, goalVal = 'maintain';
let foodSelectedName = null;
let workoutSelectedName = null;
let workoutMode = 'met'; // 'met' | 'manual'
let historySubtab = 'week';
let monthSummaryCache = {}; // 'summary:YYYY-MM' -> {"01":{c,b,bud}, ...}
let viewMonth = new Date();
viewMonth.setDate(1); // month currently shown in calendar
let viewYear = new Date().getFullYear();
let selectedDayKey = null;
let dayEditor = null; // {key:'YYYY-MM-DD', data:{foods,workouts,steps}} — a past day open for editing
let editingFoodId = null; // set while the food modal is editing an existing entry rather than adding a new one
let editingWorkoutId = null; // same, for the workout modal
/* ---- pedometer state ---- */
...
async function storeSet(key, val) {
    try {
        localStorage.setItem('cal_' + key, JSON.stringify(val));
    }
    catch (e) {
...
function renderToday() {
    ...
    if (pct >= 0.8 && pct < 1.0) {
        ...
        statusColor = 'rgba(255,138,98,0.16)';
        statusFg = 'var(--burn)';
    }
    if (pct >= 1.0) {
        grad = 'url(#gradOver)';
        statusText = 'Перебор бюджета';
        statusColor = 'rgba(255,92,122,0.16)';
        statusFg = 'var(--over)';
    }
    ...
    if (remaining >= 0) {
        big.textContent = fmt(remaining);
        lbl.textContent = 'осталось ккал';
    }
    else {
        big.textContent = fmt(Math.abs(remaining));
        lbl.textContent = 'перебор, ккал';
    }
    const st = el('ring-status');
    st.textContent = statusText;
    st.style.background = statusColor;
    st.style.color = statusFg;
...
function registerWalkTime(target) {
    const now = Date.now();
    if (pedoLastStepAt > 0) {
        const gap = now - pedoLastStepAt;
        if (gap > 0 && gap <= WALK_GAP_MS)
            target.walkMs = (target.walkMs || 0) + gap;
    }
    pedoLastStepAt = now;
}
...
async function startPedometer() {
    ...
    const motionEvent = DeviceMotionEvent;
    ...
    setTimeout(() => {
        if (pedoWatchGen !== startedAtGen)
            return; // pedometer was stopped/restarted since — ignore stale check
...
async function updateMonthSummaryForDate(dateKeyStr, totals) {
    ...
    if (!summary) {
        summary = (await storeGet(monthKey)) || {};
    }
...
async function getMonthSummary(y, m) {
    const monthKey = `summary:${y}-${pad(m)}`;
    if (monthSummaryCache[monthKey])
        return monthSummaryCache[monthKey];
...
async function renderWeekView() {
    ...
    for (const d of days) {
        ...
        if (key === todayKey())
            data = today;
        else if (historyCache[key])
            data = historyCache[key];
        else {
            data = (await storeGet('day:' + key)) || { foods: [], workouts: [], steps: 0 };
...
async function renderMonthView() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (isToday) {
            ...
            if (today.foods.length > 0 || today.workouts.length > 0)
                entry = { c: Math.round(consumed), b: Math.round(burned), bud: Math.round(budget) };
        }
    ...
    grid.innerHTML = html;
    if (selectedDayKey)
        showDayDetail(selectedDayKey, summary);
}
...
async function renderYearView() {
    ...
    const monthSummaries = await Promise.all(Array.from({ length: 12 }, (_, i) => getMonthSummary(viewYear, i + 1)));
    ...
    days.forEach((d, idx) => {
        ...
        const key = dateKey(d);
        if (inYear && d.getMonth() !== lastMonthSeen) {
        ...
        const entry = ms[dd];
        if (!entry) {
            cellsHtml += `<div class="year-cell" title="${key}: нет данных"></div>`;
            return;
        }
        const remaining = entry.bud - entry.c;
...
function fillProfileForm() {
    if (profile) {
        ...
        activityVal = profile.activity;
        goalVal = profile.goal;
...
function allFoods() {
    return [...FOOD_DB, ...customFoods.map((f) => [f.name, f.kcal100g])];
}
...
function selectFood(name, kcal100g, grams) {
    ...
    if (foodSelBox.scrollIntoView)
        foodSelBox.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
...
async function searchFoodOnlineHandler() {
    const q = el('food-search').value.trim();
    if (!q) {
        showToast('Сначала введите название продукта');
        return;
    }
...
function manualWorkoutEntry() {
    ...
    workoutSelectedName = q || 'Своя активность';
    workoutMode = 'manual';
...
async function confirmAddWorkout() {
    ...
    if (kcal <= 0) {
        showToast('Укажите значение больше 0');
        return;
    }
    const target = currentAddTarget();
    if (editingWorkoutId) {
